...
def get_positions():
    ...
    ).group_by(Trade.account_id).subquery()
    ...
    positions = db.session.query(
        ...
        account_totals, Trade.account_id == account_totals.c.account_id
    ).filter(
        Trade.trade_date == query_date
    ).group_by(
        Trade.account_id, Trade.ticker, account_totals.c.total_value
    ).all()
    ...
    for pos in positions:
        percentage = (float(pos.ticker_value) / float(pos.total_value) * 100) if pos.total_value else 0
...
def get_alarms():
    ...
    violations = db.session.query(
        ...
        (func.sum(func.abs(Trade.market_value)) / func.abs(account_totals.c.total_value) * 100) > 20
    ).all()
    ...
    for violation in violations:
        percentage = (float(violation.ticker_value) / float(violation.total_value) * 100) if violation.total_value else 0
